...
//template<typename TYPE>
//inline void EngineQuadtree::CollectIntersections(std::map<float, GameObject*>& objects, const TYPE& primitive) const
//{
//	if (root != nullptr)
...
template<typename TYPE>
inline void EngineQuadtree::CollectIntersections(std::vector<GameObject*>& objects, const TYPE& primitive) const
{
	if (root != nullptr)
		root->CollectIntersections(objects, primitive);
}
...
//		{
//			if (primitive.Intersects((*it)->getBoundingBox(), hit_near, hit_far))
//				objects[hit_near] = *it;
//		}
//
//		for (int i = 0; i < 4; ++i)
