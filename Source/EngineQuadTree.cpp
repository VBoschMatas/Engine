...
void QuadtreeNode::RedistributeChilds()
{
	...
	for (std::list<GameObject*>::iterator it = objects.begin(); it != objects.end();)
	{
		...
		AABB new_box(go->world_bbox.MinimalEnclosingAABB());
...
//	for (std::list<GameObject*>::const_iterator it = this->objects.begin(); it != this->objects.end(); ++it)
//	{
//		float dist = origin.DistanceSq((*it)->Transform()->getWorldPos());
//		objects[dist] = *it;
//	}
//
//	for (int i = 0; i < 4; ++i)
...
void EngineQuadtree::SetBoundaries(const AABB& box)
{
	Clear();
...
void EngineQuadtree::Insert(GameObject* go)
{
	if (root != nullptr)
	{
		if (go->world_bbox.MinimalEnclosingAABB().Intersects(root->box))
			root->Insert(go);
	}
}

void EngineQuadtree::Erase(GameObject* go)
{
	if (root != nullptr)
		root->Erase(go);
}
...
void EngineQuadtree::CollectBoxes(std::vector<const QuadtreeNode*>& nodes) const
{
	if (root != nullptr)
		root->CollectBoxes(nodes);
}

void EngineQuadtree::CollectObjects(std::vector<GameObject*>& objects) const
{
	if (root != nullptr)
